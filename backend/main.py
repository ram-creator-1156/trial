...
# ═════════════════════════════════════════════════════════════════════════════
#  ENDPOINT 0 — GET /api/next_match/{exporter_id}
#  Returns the single best importer recommendation with full explainability.
# ═════════════════════════════════════════════════════════════════════════════

@app.get(
    "/api/next_match/{exporter_id}",
    tags=["Matchmaking"],
    summary="Top-1 recommended importer for an exporter (with full explainability_dict)",
)
def get_next_match(
    exporter_id: str,
    skip: int = Query(0, ge=0, description="How many top matches to skip (for paging through cards)"),
) -> dict:
    """
    Returns **the single next-best importer** for the given exporter — the
    core recommendation powering the Swipe UI.
    ...
    ```
    The `explainability_dict` maps each pillar key to its sub-score and a list
    of human-readable reasoning strings — suitable for direct display in the
    Streamlit radar chart / metrics view.
    """
    engine   = _get_engine()
    clean_id = _clean_id(exporter_id)

    df = None
    for try_id in dict.fromkeys([exporter_id, clean_id]):
        try:
    ...
    if df is None:
        raise HTTPException(
            status_code=404,
            detail=f"Exporter '{exporter_id}' not found. Check /api/exporters.",
        )

    if df.empty or skip >= len(df):
        raise HTTPException(
            status_code=404,
            detail=f"No match available at skip={skip} for exporter '{exporter_id}'.",
        )

    row = df.iloc[skip].to_dict()
    ms  = _to_match_score(row)

    # Build the explainability_dict in a UI-friendly shape
    exp_raw = ms.explanation or {}
    explainability_dict = {
        "product_compatibility": {
            "label":   "Product Compatibility",
            "max_pts": 30,
            "score":   round(ms.p1_product, 2),
            "pct":     round(ms.p1_product / 30 * 100, 1),
            "reasons": (exp_raw.get("1_product_compatibility") or {}).get("reasons", []),
        },
        "geography_fit": {
            "label":   "Geography Fit",
            "max_pts": 20,
            "score":   round(ms.p2_geography, 2),
            "pct":     round(ms.p2_geography / 20 * 100, 1),
            "reasons": (exp_raw.get("2_geography_fit") or {}).get("reasons", []),
        },
        "high_intent_signals": {
            "label":   "High-Intent Signals",
            "max_pts": 30,
            "score":   round(ms.p3_signals, 2),
            "pct":     round(ms.p3_signals / 30 * 100, 1),
            "reasons": (exp_raw.get("3_high_intent_signals") or {}).get("reasons", []),
        },
        "trade_activity": {
            "label":   "Trade Activity",
            "max_pts": 20,
            "score":   round(ms.p4_activity, 2),
            "pct":     round(ms.p4_activity / 20 * 100, 1),
            "reasons": (exp_raw.get("4_trade_activity") or {}).get("reasons", []),
        },
    }
    ...
    return {
        ...
        "match": {
            "importer_buyer_id": ms.importer_buyer_id,
            "total_score":       ms.total_score,
            "grade":             ms.grade,
            "p1_product":        ms.p1_product,
            "p2_geography":      ms.p2_geography,
            "p3_signals":        ms.p3_signals,
            "p4_activity":       ms.p4_activity,
            "explainability_dict": explainability_dict,
        },
    }
